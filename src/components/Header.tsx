...
import { FaChevronDown, FaShoppingCart, FaSearch, FaUser, FaSignOutAlt, FaReceipt, FaLock, FaUserCircle } from 'react-icons/fa';
            ...
            {session?.user ? (
              <div className="relative user-menu-container">
                <button 
                  ...
                  className="text-white/90 hover:text-white transition-all duration-300 p-2 rounded-lg hover:bg-white/10 flex items-center gap-2"
                >
                  <div className="w-8 h-8 rounded-full bg-etpc-blue flex items-center justify-center text-white text-sm font-bold">
                    {session.user.name?.charAt(0).toUpperCase() || 'U'}
                  </div>
                  <span className="font-medium">{session.user.name?.split(' ')[0] || 'Conta'}</span>
                  <FaChevronDown className={`ml-1 transition-transform duration-300 ${userMenuOpen ? 'rotate-180' : ''}`} />
                </button>
                {userMenuOpen && (
                  <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl z-20 overflow-hidden animate-fadeIn">
                    {/* User Info Header */}
                    <div className="px-4 py-3 bg-gradient-to-r from-etpc-blue to-etpc-blue-dark text-white">
